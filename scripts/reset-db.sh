...
DB_CONTAINER=$(docker ps -q -f "name=foodcore-order-ms-db")
if [ -z "$DB_CONTAINER" ]; then
  echo "-> O banco de dados não está em execução. Iniciando..."
  cd "$PROJECT_ROOT/docker"
  docker-compose up -d order-ms-db
  sleep 10
  DB_CONTAINER=$(docker ps -q -f "name=foodcore-order-ms-db")
fi

echo "-> Dropando e recriando o banco de dados..."
docker exec $DB_CONTAINER psql -U postgres -c "DROP DATABASE IF EXISTS order;"
docker exec $DB_CONTAINER psql -U postgres -c "CREATE DATABASE order;"
