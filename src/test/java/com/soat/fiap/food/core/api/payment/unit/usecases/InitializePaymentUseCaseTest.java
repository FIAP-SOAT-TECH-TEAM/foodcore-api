...
@DisplayName("InitializePaymentUseCase - Testes Unitários")
class InitializePaymentUseCaseTest {
    ...
    @Test
    @DisplayName("Deve retornar pagamento existente quando já inicializado")
    ...
    @Test
    @DisplayName("Deve criar novo pagamento quando não existe")
    void shouldCreateNewPaymentWhenNotExists() {
        ...
        when(acquirerGateway.generateQrCode(any(OrderCreatedInput.class), any(LocalDateTime.class))).thenReturn(qrCodeValue);
        ...
        assertEquals("00020126580014BR.GOV.BCB.PIX0136123e4567-e12b-12d1-a456-426655440000", result.getQrCode().value());
    }

    @Test
    @DisplayName("Deve criar pagamento com data de expiração correta")
