...
SELECT 'ADMIN',
       ...
       now(),
       now() WHERE NOT EXISTS (SELECT 1 FROM roles WHERE name = 'ADMIN');
...
SELECT 'USER',
       ...
       now(),
       now() WHERE NOT EXISTS (SELECT 1 FROM roles WHERE name = 'USER');

-- Criação do Role GUEST
INSERT INTO roles (name, description)
VALUES ('GUEST', 'Usuário convidado') ON CONFLICT (name) DO NOTHING;
...
SELECT 'Admin Sistema',
       'admin',
       'admin@fastfood.com',
       '$2a$12$kl2iMwHXVtymLAza2P2U/uByd5LjRcxrkkF5Y4I.rst.pens6x4Iy',
       true,
       1,
       now(),
...
SELECT 'João Silva',
       ...
       2,
       now(),
       now() WHERE NOT EXISTS (SELECT 1 FROM users WHERE email = 'joao@email.com');
...
SELECT 'Maria Oliveira',
       ...
       2,
       now(),
       now() WHERE NOT EXISTS (SELECT 1 FROM users WHERE email = 'maria@email.com');
...
SELECT 'Ana Souza',
       ...
       2,
       now(),
       now() WHERE NOT EXISTS (SELECT 1 FROM users WHERE email = 'ana@email.com');
