...
/**
 * Classe utilitária responsável por mapear objetos entre {@link OrderItemRequest} e {@link OrderItemInput}.
 */
public class OrderItemMapper {

    /**
     * Converte um {@link OrderItemRequest} (camada web.api) em um {@link OrderItemInput} (camada de aplicação).
     *
     * @param request O DTO de item do pedido recebido via API.
    ...
    /**
     * Converte um {@link OrderItemInput} (camada de aplicação) em uma entidade de domínio {@link OrderItem}.
     *
     * @param input O DTO de entrada representando o item do pedido.
    ...
    public static OrderItem toDomain(OrderItemInput input) {
        return new OrderItem(
                input.productId(),
                input.name(),
                new OrderItemPrice(input.quantity(), input.unitPrice()),
                input.observations()
        );
    }
}