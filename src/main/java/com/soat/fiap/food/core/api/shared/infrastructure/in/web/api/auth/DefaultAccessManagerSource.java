...
public class DefaultAccessManagerSource implements AccessManagerSource {
	...
	public void validateAccess(String userId) {
		...
		if (role == null || role.isEmpty()) {
			log.info("Negando acesso para o usuário: {}, pela impossibilidade de determinar sua Role, no recurso cujo dono é: {}", currentUserId, userId);
		...
		if (!currentUserId.equals(userId) && !"ADMIN".equalsIgnoreCase(role)) {
			log.info("Negando acesso para o usuário: {}, de Role: {}, no recurso cujo dono é: {}", currentUserId, role, userId);
